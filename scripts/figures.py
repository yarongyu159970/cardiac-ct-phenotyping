...
MODEL_C_FORMULA = "age + C(gender) + SBP + C(`Antihypertensive medication`) + TC + HDL + C(smoking) + C(DM) + C(CADRADS) + C(Cluster)"
PENALIZER = 0.01
RISK_TIMES = np.arange(0, 61, 10, dtype=float)
KEY_COLUMNS = ("MACET", "TIME", "age", "SBP", "TC", "HDL", "CAD-RADS")
ENDPOINT_SPECS = [
...
def prepare_derivation(path: str | Path) -> pd.DataFrame:
    ...
    needed = {"ID", "Cluster", "gender", "Antihypertensive medication", "smoking", "DM", *KEY_COLUMNS}
    ...
    if data[["ID", "MACET", "TIME", "CAD-RADS", "Cluster"]].isna().any().any():
        raise ValueError("ID, MACET, TIME, CAD-RADS and Cluster must be complete")
    return _finish(data)
...
def phenotype_heatmap_table(
    ...
    for output_name, source_name in (
        ...
        ("Total plaque volume", "Total plaque volume"),
        ("Age", "age"),
        ("BMI", "BMI"),
        ("SBP", "SBP"),
        ("Fasting Glucose", "FG"),
    ...
    categorical_spec = [
        ...
        ("Antiplatelet therapy", {0: "Antiplatelet_No", 1: "Antiplatelet_Yes"}),
        ("Antihypertensive medication", {0: "Antihypertensive_No", 1: "Antihypertensive_Yes"}),
        ("Statin", {0: "Statin_No", 1: "Statin_Yes"}),
        ("Antidiabetic medication", {0: "Antidiabetic_No", 1: "Antidiabetic_Yes"}),
        ("Nitrates", {0: "Nitrates_0", 1: "Nitrates_1"}),
        ("Antiischemic medication", {0: "Antiischemic_No", 1: "Antiischemic_Yes"}),
        ("HRP", {0: "HRP_0", 1: "HRP_1"}),
        ("LAP", {0: "LAP_0", 1: "LAP_1"}),
        ("SC", {0: "SC_0", 1: "SC_1"}),
        ("PR", {0: "PR_0", 1: "PR_1"}),
            ...
            for cluster in (1, 2, 3):
                ...
                with warnings.catch_warnings():
                    ...
                    aj = AalenJohansenFitter(seed=42).fit(
                        ...
                        data.loc[subset, "MACET"].astype(int),
