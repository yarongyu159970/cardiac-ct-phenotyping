...
CONTINUOUS = [
    ...
    ("TG, mmol/L", "TG", 1.0),
    ("TC, mmol/L", "TC", 1.0),
    ("Total plaque volume, mm3", "whole lesion volume", 1.0),
...
BINARY = [
    ...
    ("NRS, n (%)", "NRS", 1),
    ("SC, n (%)", "SC", 1),
    ("Antiplatelet therapy, n (%)", "Anti1", 1),
    ("Antihypertensive medication, n (%)", "Anti2", 1),
    ("Antidiabetic medication, n (%)", "Antidiabetic", 1),
    ("Anti-ischemic therapy, n (%)", "Antiischemic", 1),
...
def add_endpoint_indicators(frame: pd.DataFrame) -> pd.DataFrame:
    if "MACST" not in frame.columns:
        raise ValueError("Cohort table is missing MACST")
    output = frame.copy()
    code = semantic_numeric(output["MACST"])
    if code.isna().any() or not set(code.astype(int).unique()).issubset({0, 1, 2}):
        raise ValueError("MACST must be complete and coded 0, 1 or 2")
