...
BASE_COLUMNS = ["TIME", "MACET", "Cluster", "age", "gender", "SBP", "Antihypertensive medication", "TC", "HDL", "smoking", "DM", "CAD-RADS"]


def interaction_formulas(medication: str) -> tuple[str, str]:
    medication = f"`{medication}`" if " " in medication else medication
...
def prepare_analysis_data(frame: pd.DataFrame, medications: list[str], name: str) -> pd.DataFrame:
    ...
    data["composite_event"] = data["MACET"].ne(0).astype(int)
    data["hard_event"] = data["MACET"].eq(1).astype(int)
    return data
...
def main() -> None:
    ...
    settings = {
        "endpoints": {"Composite_MACE": "MACET != 0", "Hard_MACE": "MACET == 1"},
