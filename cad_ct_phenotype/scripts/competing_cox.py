...
CATEGORICAL = ["gender", "Anti2", "smoking", "DM", "CADRADS", "Cluster"]
...
def weights(train: pd.DataFrame, evaluation: pd.DataFrame) -> list[tuple[np.ndarray, np.ndarray, np.ndarray]]:
    ...
    tt, st = train.TIME.to_numpy(), train.MACST.to_numpy()
    t, s = evaluation.TIME.to_numpy(), evaluation.MACST.to_numpy()
