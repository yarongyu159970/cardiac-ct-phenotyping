...
if (length(setdiff(c("ID", "TIME", "MACST"), names(cohort))) > 0) stop("Cohort is missing ID, TIME or MACST.")
...
if (anyNA(data[, c("ID", "TIME", "MACST", "Cluster")])) stop("ID, TIME, MACST and Cluster must be complete.")
if (!all(data$MACST %in% c(0, 1, 2))) stop("MACST must be coded 0, 1 or 2.")
if (!setequal(unique(data$Cluster), c(1, 2, 3))) stop("All three clusters must be present.")

fit <- cmprsk::cuminc(
  ftime = as.numeric(data$TIME),
  fstatus = as.integer(data$MACST),
...
event_counts <- aggregate(list(n = data$ID), by = list(Cluster = data$Cluster, event_code = data$MACST), FUN = length)
