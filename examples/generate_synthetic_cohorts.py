...
def make_cohort(name: str, seed: int, n: int = 240) -> pd.DataFrame:
    ...
    d["Total plaque volume"] = measurement([80.0, 230.0, 1000.0], 25.0)
    d["low attenuation volume"] = d["Total plaque volume"] * rng.uniform(0.01, 0.12, n)
    d["calcified volume"] = d["Total plaque volume"] * 0.30
    d["fibrotic volume"] = d["Total plaque volume"] * 0.40
    d["fibrous fatty volume"] = (
        d["Total plaque volume"] - d["low attenuation volume"] - d["calcified volume"] - d["fibrotic volume"]
    )
    ...
    if name == "derivation":
        d["Cluster"] = labels
    d["MACET"] = rng.permutation(np.tile([0, 0, 1, 2], n // 4))
    d["TIME"] = np.where(d.MACET.eq(0), rng.uniform(18, 96, n), rng.uniform(3, 72, n))
    return d
