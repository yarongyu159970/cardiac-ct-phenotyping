...
| `MACET` | 0 = no event, 1 = hard MACE, 2 = soft MACE. Composite MACE is `MACET != 0`; hard MACE is `MACET == 1` |
...
* Plaque volumes in mm³: `Total plaque volume`, `low attenuation volume`, `calcified volume`,
...
`predict.py` requires the 13 features `Total plaque volume`, `CT-FFR-RCA`, `LAD_MBF`, `RCA_PCBV`, `TC`,
